      ...
      <section style={{ position: 'relative', minHeight: '92vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px 24px', overflow: 'hidden' }}>
        ...
        <div className="fade-up" style={{ textAlign: 'center', maxWidth: 820, position: 'relative' }}>
          ...
          <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 64 }}>
            <button className="btn-primary" onClick={() => nav('/check')} style={{ fontSize: 17, padding: '15px 38px' }}>
              Check My Resume Free →
            </button>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 22px', background: 'var(--bg2)', border: '1px solid var(--border2)', borderRadius: 12, fontSize: 14, color: 'var(--text2)' }}>
          ...
          <div style={{ display: 'flex', gap: 48, justifyContent: 'center', flexWrap: 'wrap' }}>
            {[['100%', 'Always free'], ['Real', 'Keyword matching'], ['0', 'Fake scores'], ['60s', 'To get results']].map(([n, l]) => (
              <div key={l}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: 30, fontWeight: 800, color: 'var(--text)', lineHeight: 1 }}>{n}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
            {STEPS.map((s, i) => (
              <div key={s.n} className="card" style={{ position: 'relative', overflow: 'hidden' }}>
                ...
                <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 19, fontWeight: 700, marginBottom: 10 }}>{s.title}</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16 }}>
            {FEATURES.map(f => (
              <div key={f.title} className="card" style={{ cursor: 'default' }}>
                ...
                <p style={{ fontSize: 14, lineHeight: 1.75 }}>{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '64px 24px', background: 'var(--bg2)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1120, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
            {ROLES.map(r => (
              <span key={r} style={{ padding: '8px 20px', background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 100, fontSize: 14, color: 'var(--text2)', fontFamily: 'var(--font-display)', fontWeight: 500 }}>{r}</span>
            ))}
          </div>
        </div>
      </section>
